...
ALLOWED_EXTENSIONS = {
    '.md', '.pdf', '.docx', '.csv', '.tsv', '.json', '.xlsx', '.parquet'
}
...
def check_git():
    try:
        ...
        print("ERROR: 'git' command not found. Please install Git and add it to PATH.")
        return False
...
def main():
    try:
        if clone_repo():
            ...
            report_path = generate_report(files, sim_pairs, True)
            if create_zip():
                print("\nSUCCESS: Extraction complete.")
                print(f"Output: {os.path.abspath(FINAL_ZIP)}")
